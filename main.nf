...
process IgblastDataCheck { // cannot be IgblastDataCheck outside of process because the files are present in the docker
    label 'immcantation'
...
process ParseCoordinates{
    ...
    path "ParseCoordinates.log", emit: parse_coord_log_ch
...
process ParseDbFiltering {
    ...
    if [[ "${igblast_aa}" == "true" ]]; then # if igblast_aa is true, then the productive column is empty because aa sequences means productive ones
        ...
        if [ -s *_productive-F.tsv ]; then # see above for -s
            ...
            echo -e "\n\nWARNING: EMPTY unproductive_seq.tsv FILE RETURNED FOLLOWING THE ParseDbFiltering PROCESS\n\n" |& tee -a ParseDb_filtering.log
...
process SeqNameReplacement {
    ...
    path "SeqNameReplacement.log", emit: seq_name_replacement_log_ch

    script:
    """
    #!/bin/bash -ue
    FILENAME=\$(basename -- ${translation_ch}) # recover a file name without path
    echo -e "\\n\\n################################\\n\\n\$FILENAME\\n\\n################################\\n\\n" |& tee -a SeqNameReplacement.log
    echo -e "WORKING FOLDER:\\n\$(pwd)\\n\\n" |& tee -a SeqNameReplacement.log
    # check first that the data file does not have the second column name starting by "initial_". Otherwise, will create unproper behavior in Donut
...
// Add the dist_to_nearest values and gene columns in the SeqNameReplacement file, thereby creating the productive_seq.tsv file
...
process DataAssembly {
    label 'immcantation'
    publishDir path: "${out_path}/files", mode: 'copy', pattern: "{productive_seq.tsv}", overwrite: false
    publishDir path: "${out_path}/reports", mode: 'copy', pattern: "{DataAssembly.log}", overwrite: false
    cache 'true'
    ...
    ' |& tee -a DataAssembly.log
    """
}


process MetadataCheck { // cannot be in germ_tree_vizu because I have to use the clone_assigned_seq.tsv file for the check
    label 'immcantation'
    cache 'true'
    ...
    if [[ "${meta_file}" != "NULL" ]] ; then
        Rscript -e '
            df <- read.table("${productive_ch}", header = TRUE, sep = "\\t")
            meta <- read.table("./${meta_file}", sep = "\\t", header = TRUE)
            ...
            if( ! meta_seq_names %in% names(meta)){
                stop(paste0("\\n\\n============\\n\\nERROR IN THE MetadataCheck PROCESS OF NEXTFLOW\\nIF THE meta_path PARAMETER IS NOT \\"NULL\\", THEN THE meta_seq_names PARAMETER MUST BE A COLUMN NAME OF THE METADATA FILE.\\n\\n============\\n\\n"), call. = FALSE)
            }
            ...
            if(names(df)[1] != "sequence_id"){
                stop(paste0("\\n\\n============\\n\\nINTERNAL ERROR IN THE MetadataCheck PROCESS OF NEXTFLOW\\nIF THE meta_path PARAMETER IS NOT \\"NULL\\", THEN THE TABLE GENERATED USING THE sample_path PARAMETER MUST CONTAIN sequence_id AS FIRST COLUMN NAME.\\n\\n============\\n\\n"), call. = FALSE)
            }
            if(meta_name_replacement != "NULL"){
                if( ! meta_name_replacement %in% names(meta)){
...
process DistToNearest {
    ...
    #!/bin/bash -ue
    Rscript -e '
         # WEIRD stuf: if db alone is returned, and if distToNearest_ch is used for the CloneAssignment process and followings, everything is fine. But if db3 is returned with db3 <- data.frame(db, dist_nearest = db2\$dist_nearest) or db3 <- data.frame(db, caca = db2\$dist_nearest) or data.frame(db, caca = db\$sequence_id) or db3 <- data.frame(db, caca = as.numeric(db2\$dist_nearest)) or db3 <- data.frame(db[1:3], caca = db\$sequence_id, db[4:length(db)]), the get_germ_tree process cannot make trees, while the productive.tsv seem identical at the end, between the use of db or db3, except that the clone_id order is not the same
...
process DistanceHist {
    label 'immcantation'
    publishDir path: "${out_path}/figures/png", mode: 'copy', pattern: "{*.png}", overwrite: false
    publishDir path: "${out_path}/figures/svg", mode: 'copy', pattern: "{*.svg}", overwrite: false
    publishDir path: "${out_path}/reports", mode: 'copy', pattern: "{DistanceHist.log}", overwrite: false
    cache 'true'
...
process HistogramAssembly {
    label 'r_ext'
    publishDir path: "${out_path}/files", mode: 'copy', pattern: "{seq_distance.pdf}", overwrite: false
    publishDir path: "${out_path}/reports", mode: 'copy', pattern: "{HistogramAssembly.log}", overwrite: false
    cache 'true'
    ...
    ' |& tee -a HistogramAssembly.log
    """
}


process CloneAssignment {
    label 'immcantation'
    ...
    if [[ -s ${productive_ch} ]]; then # see above for -s
        DefineClones.py -d ${productive_ch} --act ${clone_strategy} --model ${clone_model} --norm ${clone_normalize} --dist ${clone_distance} --fail |& tee -a CloneAssignment.log
        Rscript -e '
        ...
        if [ -s *_clone-fail.tsv ]; then # see above for -s
            cp *_clone-fail.tsv non_clone_assigned_sequence.tsv |& tee -a CloneAssignment.log
            Rscript -e '
            ...
            echo -e "\n\nNOTE: EMPTY non_clone_assigned_sequence.tsv FILE RETURNED FOLLOWING THE CloneAssignment PROCESS\n\n" |& tee -a CloneAssignment.log
            echo -n "" | cat > non_clone_assigned_sequence.tsv
        fi
    else
        echo -e "\\n\\n========\\n\\nINTERNAL ERROR IN NEXTFLOW EXECUTION\\n\\nEMPTY FILE GENERATED BY THE Translation PROCESS\\nCHECK THE CloneAssignment.log AND *_productive-F.tsv FILES IN THE report FOLDER INSIDE THE OUTPUT FOLDER\\n\\nPLEASE, REPORT AN ISSUE HERE https://gitlab.pasteur.fr/gmillot/repertoire_profiler/-/issues OR AT gael.millot<AT>pasteur.fr.\\n\\n========\\n\\n"
        exit 1
    fi
...
process SplitByClones { // split the file into multiple files according to the clone_id column
    label 'immcantation'
    cache 'true'
...
process ClosestGermline {
    ...
    VDJ_FILES=\$(awk -v var1="${igblast_variable_ref_files}" -v var2="\${REPO_PATH}" 'BEGIN{ORS=" " ; split(var1, array1, " ") ; for (key in array1) {print var2"/"array1[key]}}')
    # end variables
    CreateGermlines.py -d \$FILENAME -g dmask --cloned -r \${VDJ_FILES} |& tee -a ClosestGermline.log
...
process MutationLoad {
    ...
    Rscript -e '
        ...
        if(length(args) != length(tempo.arg.names)){
          tempo.cat <- paste0("\\n\\n========\\n\\nINTERNAL ERROR IN THE NEXTFLOW EXECUTION OF THE MutationLoad PROCESS\\n THE NUMBER OF ELEMENTS IN args (", length(args),") IS DIFFERENT FROM THE NUMBER OF ELEMENTS IN tempo.arg.names (", length(tempo.arg.names),")\\nargs:", paste0(args, collapse = ","), "\\ntempo.arg.names:", paste0(tempo.arg.names, collapse = ","), "\\n\\nPLEASE, REPORT AN ISSUE HERE https://gitlab.pasteur.fr/gmillot/repertoire_profiler/-/issues OR AT gael.millot<AT>pasteur.fr.\\n\\n========\\n\\n")
          stop(tempo.cat)
        }
    ...
    ' "\${FILENAME}" |& tee -a MutationLoad.log
...
process get_germ_tree {
    ...
    if [[ ! -s ${mutation_load_ch} ]]; then
        echo -e "\\n\\n========\\n\\nERROR IN NEXTFLOW EXECUTION\\n\\nEMPTY ${mutation_load_ch} FILE AS INPUT OF THE MutationLoad PROCESS\\nCHECK THE MutationLoad.log IN THE report FOLDER INSIDE THE OUTPUT FOLDER\\n\\n========\\n\\n"
        exit 1
    fi
...
process DonutAssembly {
    label 'r_ext'
    publishDir path: "${out_path}/files", mode: 'copy', pattern: "{donuts.pdf}", overwrite: false
    publishDir path: "${out_path}/reports", mode: 'copy', pattern: "{DonutAssembly.log}", overwrite: false
    cache 'true'
...
//      "PrintReport.log" : will contain any error or warning messages produced by rmardown::render
process PrintReport{
    label 'r_ext'

    publishDir path: "${out_path}", mode: 'copy', pattern: "{*.html}", overwrite: false
    publishDir path: "${out_path}/reports", mode: 'copy', pattern: "{PrintReport.log}", overwrite: false
    cache 'false'
...
workflow {
    ...
    ParseCoordinates.out.parse_coord_log_ch.collectFile(name: "ParseCoordinates.log").subscribe{it -> it.copyTo("${out_path}/reports")}
    ...
    // ParseDbFiltering.out.unproductive_ch.count().subscribe{n -> if ( n == 0 ){print "\n\nWARNING: EMPTY unproductive_seq.tsv FILE RETURNED FOLLOWING THE ParseDbFiltering PROCESS\n\n"}else{it -> it.copyTo("${out_path}/unproductive_seq.tsv")}} // see https://www.nextflow.io/docs/latest/script.html?highlight=copyto#copy-files
    ParseDbFiltering.out.select_ch.count().subscribe { n -> if ( n == 0 ){error "\n\n========\n\nINTERNAL ERROR IN NEXTFLOW EXECUTION\n\nO PRODUCTIVE SEQUENCE FILES FOLLOWING THE ParseDbFiltering PROCESS\n\nPLEASE, REPORT AN ISSUE HERE https://gitlab.pasteur.fr/gmillot/repertoire_profiler/-/issues OR AT gael.millot<AT>pasteur.fr.\n\n========\n\n"}}
    select_ch2 = ParseDbFiltering.out.select_ch.collectFile(name: "productive_seq_init.tsv", skip: 1, keepHeader: true)
    ParseDbFiltering.out.unselect_ch.count().subscribe { n -> if ( n == 0 ){error "\n\n========\n\nINTERNAL ERROR IN NEXTFLOW EXECUTION\n\nO UNPRODUCTIVE SEQUENCE FILES FOLLOWING THE ParseDbFiltering PROCESS\n\nPLEASE, REPORT AN ISSUE HERE https://gitlab.pasteur.fr/gmillot/repertoire_profiler/-/issues OR AT gael.millot<AT>pasteur.fr.\n\n========\n\n"}} // because an empty file must be present
    unselect_ch2 = ParseDbFiltering.out.unselect_ch.collectFile(name: "unproductive_seq.tsv", skip: 1, keepHeader: true)
    unselect_ch2.subscribe{it -> it.copyTo("${out_path}/files")}
    nb1_b = select_ch2.countLines()
    nb2_b = unselect_ch2.countLines()
    nb1_b.subscribe { n -> if ( n == 1 ){error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nO PRODUCTIVE SEQUENCE FOLLOWING THE ParseDbFiltering PROCESS\nSEE THE EMPTY productive_seq.tsv FILE AND THE unproductive_seq.tsv FILE IN THE OUTPUT FOLDER\n\n========\n\n"}}
    ...
    ParseDbFiltering.out.parseDb_filtering_log_ch.collectFile(name: "ParseDb_filtering.log").subscribe{it -> it.copyTo("${out_path}/reports")}


    Translation(
        ParseDbFiltering.out.select_ch.ifEmpty{error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE ParseDbFiltering PROCESS\n\n========\n\n"},
        igblast_aa
    )
    ...
    //DistanceHist.out.distance_hist_ch.count().subscribe { n -> if ( n == 0 ){error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE DistanceHist PROCESS\n\n========\n\n"}}



    HistogramAssembly(
        DistanceHist.out.histogram_pdf_ch.collect().ifEmpty{error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE DistanceHist PROCESS\n\n========\n\n"}
    )
   ...
   SeqNameReplacement(
    ...
    SeqNameReplacement.out.seq_name_replacement_ch.count().subscribe { n -> if ( n == 0 ){error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE SeqNameReplacement PROCESS\n\n========\n\n"}}
    seq_name_replacement_ch2 = SeqNameReplacement.out.seq_name_replacement_ch.collectFile(name: "replacement.tsv", skip: 1, keepHeader: true)
    ...
    SeqNameReplacement.out.seq_name_replacement_log_ch.collectFile(name: "SeqNameReplacement.log").subscribe{it -> it.copyTo("${out_path}/reports")}
    ...
    DataAssembly.out.productive_ch.count().subscribe { n -> if ( n == 0 ){error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE DataAssembly PROCESS\n\n========\n\n"}}

    MetadataCheck(
        DataAssembly.out.productive_ch.ifEmpty{error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE DataAssembly PROCESS\n\n========\n\n"},
        meta_file, 
        meta_seq_names, 
        meta_name_replacement,
        meta_legend
    ...
    // CloneAssignment.out.clone_ch.view()

    nb_failed_clone = CloneAssignment.out.failed_clone_ch.countLines() - 1
    

    SplitByClones(
        CloneAssignment.out.clone_ch.ifEmpty{error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE CloneAssignment PROCESS\n\n========\n\n"}
    )



    ClosestGermline(
        SplitByClones.out.clone_split_ch.flatten(), // flatten split the list into several objects (required for parallelization)
        igblast_organism, 
        igblast_variable_ref_files
    )
    ClosestGermline.out.closest_log_ch.collectFile(name: "ClosestGermline.log").subscribe{it -> it.copyTo("${out_path}/reports")}


    AddGermlineSequences(
        ClosestGermline.out.closest_ch.ifEmpty{error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE ClosestGermline PROCESS\n\n========\n\n"},
        igblast_organism, 
        igblast_variable_ref_files
    )
    ...
    //MutationLoad.out.mutation_load_ch.count().subscribe { n -> if ( n == 0 ){error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE MutationLoad PROCESS\n\n========\n\n"}}
    
    MutationLoad.out.mutation_load_log_ch.collectFile(name: "MutationLoad.log").subscribe{it -> it.copyTo("${out_path}/reports")} // 

    clone_assigned_seq = MutationLoad.out.mutation_load_ch.collectFile(name: "clone_assigned_seq.tsv", skip: 1, keepHeader: true)
    ...
    mutation_load_filtered = MutationLoad.out.mutation_load_ch.filter{ file -> file.countLines() > clone_nb_seq.toInteger() } // Only keep clonal groups that have a number of sequences superior to clone_nb_seq (variable defined in nextflow.config)
    ...
    Repertoire(
        DataAssembly.out.productive_ch,
        IgblastDataCheck.out.igblast_data_check_ch,
        cute_file
    )
    ...
    get_germ_tree(
        MutationLoad.out.mutation_load_ch,
    ...
    */
    ...
    tempo2_ch = DataAssembly.out.productive_ch.mix(DataAssembly.out.productive_ch.mix(germline_genes_ch2)) // 1 channel with 3 paths (do not use flatten() -> not list)
    ...
    //DonutAssembly.out.donut_assembly_ch.ifEmpty{error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE DonutAssembly PROCESS\n\n========\n\n"}
    DonutAssembly.out.donut_assembly_ch.count().subscribe { n -> if ( n == 0 ){error "\n\n========\n\nERROR IN NEXTFLOW EXECUTION\n\nEMPTY OUTPUT FOLLOWING THE DonutAssembly PROCESS\n\n========\n\n"}}

/*
    Reformat(
        aa_tsv_ch2,
        IgblastDataCheck.out.igblast_data_check_ch.collect() // To make sure the igblast_ref files are in the right format before executing following processes
    )
