...
process parseDb_filtering {
    ...
    """
    #!/bin/bash -ue
    if [[ ${igblast_aa} ]]; then # if igblast_aa is true, then the productive column is empty because aa sequences means productive ones
        ...
        echo "" > unproductive_seq.tsv |& tee -a ParseDb_filtering.log
    elif [[ ! -s ${tsv_ch2} ]]; then
...
process clone_assignment {
    ...
    if [[ ! -s ${translation_ch} ]]; then
        ...
        DefineClones.py -d ${translation_ch} --act set --model ${clone_model} --norm ${clone_normalize} --dist ${clone_distance} |& tee -a clone_assignment.log
    fi
    """
}
