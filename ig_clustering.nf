...
process tree_vizu {
    ...
    Rscript -e '
        ...
        if(length(tempo.list) == 0){
            ...
            if("${meta_ch}" != "NULL"){
                meta.df <- read.table("./${meta_ch}", sep = "\\t", header = TRUE)
            }
            ...
            for(i3 in 1:length(plots)){
                ...
                if(is.null(leaf.nodes)){
                    stop("\\n\\n========\\n\\nINTERNAL ERROR IN NEXTFLOW EXECUTION\\n\\nEMPTY leaf.nodes OBJECT GENERATED BY THE tree_vizu PROCESS\\n\\n========\\n\\n")
                }
                ...
                if("${meta_ch}" == "NULL"){
                    ...
                    assign(paste0(tempo.gg.name, tempo.gg.count <- tempo.gg.count + 1), ggtree::"%<+%"(
                        ggtree::ggtree(trees\$trees[[i3]], layout = "${tree_kind}"),
                        meta.df
                    ))
                    assign(paste0(tempo.gg.name, tempo.gg.count <- tempo.gg.count + 1), ggtree::geom_tippoint(
                        ggplot2::aes(fill = tempo.names, size = ${tree_meta_path_names}),
                        pch = ${tree_leaf_shape}
                    ))
                }
                ...
                assign(paste0(tempo.gg.name, tempo.gg.count <- tempo.gg.count + 1), ggplot2::scale_discrete_manual(
                    aesthetics = "fill", 
                    name = NULL, 
                    values = c("black", ifelse("${tree_leaf_color}" == "NULL", "tomato", "${tree_leaf_color}")),
                ...
                if(${tree_legend} == "FALSE"){
                    assign(paste0(tempo.gg.name, tempo.gg.count <- tempo.gg.count + 1), ggplot2::guides(fill = "none"))
                }
                plots[[i3]] <- eval(parse(text = paste(paste0(tempo.gg.name, 1:tempo.gg.count), collapse = " + ")))
                            save(list = ls(), file = "./caca.RData")
