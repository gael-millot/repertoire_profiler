...
**Sequence alignment** are available in the [alignments/nuc](./alignments/nuc) folder for nucleotide alignments and in the [alignments/aa](./alignments/aa) folder for amino acids alignments. Folders include:<br /><ul><li>germline clustering (clone ID).<br /></li><li>allele reannotation (germinal_v_call and germinal_j_call columns).<br /></li><li>mutation load.<br /></li><li>distance and sequence nickname (annotation from the metadata file) added.<br></ul>

The HTML files present the alignment visualizations, while the FASTA files contain the corresponding sequences for each clonal group. The *.gff* files contain the associated region coordinates per clonal group.<br>
