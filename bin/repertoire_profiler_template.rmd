...
Some sequences from the productive file can **fail to be clone assigned**, and will not be in the clone-assigned_seq.tsv file. 

Number of [clone-assigned sequences](files/clone-assigned_seq.tsv) : ```r params$nb_clone-assigned```

Number of [failed clone-assigned sequences](files/non_clone_assigned_sequence.tsv) : ```r params$nb_failed_clone```

Passing ratio : ```r base::round(params$nb_clone-assigned / (params$nb_clone-assigned + params$nb_failed_clone), 2)```
