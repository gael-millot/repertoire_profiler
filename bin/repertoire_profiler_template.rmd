...
if(params$nb_productive == -1){
    if(params$nb_wanted != -1){
...
Number of annotated sequences: ```r params$nb_igblast```
<br>
Number of sequences failing annotation: ```r params$nb_unigblast```
<br>
Total: ```r params$nb_igblast + params$nb_unigblast```

Success ratio: ```r base::round(params$nb_igblast / (params$nb_igblast + params$nb_unigblast), 2)```
...
**Wanted sequences** are productive sequences removed from unwanted sequences, according to the parameters of the <i>nextflow.config</i> file:<br><ul><li><code>igblast_organism</code>:    `r params$igblast_organism`<br></li><li><code>igblast_loci</code>:    `r params$igblast_loci`<br></li><li><code>igblast_B_heavy_chain</code>:    `r params$igblast_B_heavy_chain`<br></li><li><code>igblast_B_lambda_chain</code>:    `r params$igblast_B_lambda_chain`<br></li><li><code>igblast_B_kappa_chain</code>:    `r params$igblast_B_kappa_chain`<br></li><li><code>igblast_T_alpha_chain</code>:    `r params$igblast_T_alpha_chain`<br></li><li><code>igblast_T_beta_chain</code>:    `r params$igblast_T_beta_chain`<br></li><li><code>igblast_T_gamma_chain</code>:    `r params$igblast_T_gamma_chain`<br></li><li><code>igblast_T_delta_chain</code>:    `r params$igblast_T_delta_chain`<br></ul>
<br>
...
if(params$nb_productive != -1){
    ...
    cat('
Number of productive sequences: ', params$nb_productive
    )
    if(params$nb_productive > 0){
        cat('
...
\n**(Note: Table truncated to 10 rows. Full file available in the productive_seq.tsv file.)**\n\n
    ...
    cat('
<br>
Number of unproductive sequences: ', params$nb_unproductive
    )
    if(params$nb_unproductive > 0){
        cat('
&nbsp; <details class="inline"><summary style="color: #1a73e8; font-family: Arial, sans-serif; font-size: 1em; font-weight: bold;">view</summary>
...
'\n**(Note: Table truncated to 10 rows. Full file available in the unproductive_seq.tsv file.)**\n\n
    ...
    cat('
<br>
Number of wanted sequences: ', params$nb_wanted    
    )
    if(params$nb_wanted > 0){
        cat('
&nbsp; <details class="inline"><summary style="color: #1a73e8; font-family: Arial, sans-serif; font-size: 1em; font-weight: bold;">view</summary>
...
\n**(Note: Table truncated to 10 rows. Full file available in the wanted_seq.tsv file.)**\n\n
    ...
    cat('
Number of unwanted sequences: ', params$nb_unwanted
    )
    if(params$nb_unwanted > 0){
        cat('
&nbsp; <details class="inline"><summary style="color: #1a73e8; font-family: Arial, sans-serif; font-size: 1em; font-weight: bold;">view</summary>
...
\n**(Note: Table truncated to 10 rows. Full file available in the unwanted_seq.tsv file.)**\n\n
    ...
    cat('
<br>
Total (wanted + unproductive + unwanted): ', params$nb_wanted + params$nb_unproductive + params$nb_unwanted, '

Productive / Total ratio: ', base::round(params$nb_productive / (params$nb_wanted + params$nb_unproductive + params$nb_unwanted), 2), '

Wanted / Productive ratio: ', base::round(params$nb_wanted / params$nb_productive, 2), '
...
```{r, echo=FALSE, results='asis'}
if(params$nb_productive != -1 && params$nb_wanted != -1){
...
Threshold value set in the `clone_distance` parameter of the *nextflow.config* file: ', params$clone_distance, '
...
Number of sequences used in the distance matrix (from wanted_seq.tsv): ', params$nb_wanted, '
<br>
Number of sequences that could not be used in the distance matrix: ', nb_dist_ignored, '

Sequence ratio: ', base::round((params$nb_wanted - nb_dist_ignored)/ params$nb_wanted, 2), '
...
```{r, echo=FALSE, results='asis'}
#remove work directory from current directory
if(params$warning_collect == ""){
    cat('No warning message reported.')
}else{
    cat(params$warning_collect)
}
```
