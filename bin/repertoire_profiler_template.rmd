...
[Succes](tsv/igblast_aligned_seq_name.tsv) number of sequences: ```r params$nb_seq_aligned```

[Failed](tsv/igblast_unaligned_seq_name.tsv) number of sequences: ```r params$nb_seq_not_aligned```
...
**Productive sequences** are sequences with:<br /><ul><li>an open reading frame (start and stop codon).<br /></li><li>no defect in the start codon, splicing sites or regulatory elements.<br /></li><li>no internal stop codons.<br /></li><li>in-frame junction between regions.<br></ul>
...
<br>
Only V, J and C genes are displayed here. See the [donuts.pdf](pdf/donuts.pdf) file in the pdf folder for the complete results, including the alleles (different copies of a same gene) proportions.
...
Click on a figure to enlarge. Reclick to go back to the report.
<br>
Only V, J and C genes are displayed here. See the [gene_repertoire.pdf](pdf/gene_repertoire.pdf) and [allele_repertoire.pdf](pdf/allele_repertoire.pdf) files in the pdf folder for the complete results, including the alleles (different copies of a same gene) counts.
...
<span style="color:green">*non_clone_assigned_sequence.tsv file in the [tsv](./tsv) folder*</span>
```{r, echo=FALSE, results='asis'}
df <- read_tsv_with_dummy("tsv/non_clone_assigned_sequence.tsv")
if (is.data.frame(df) && attr(df, "truncated")) {
  cat("\n**(Note: Table truncated to 10 rows. Full file available in the non_clone_assigned_sequence.tsv file.)**\n\n")
}
kable(df, format = "markdown", row.names = FALSE)
```
</details>
...
### Alignments of the variable regions for each clonal group

<br>

**Sequence Alignments** are available in the [alignments/nuc](./alignments/nuc) folder for nucleotide alignments and in the [alignments/aa](./alignments/aa) folder for amino acids alignments. Such alignments are performed using [Abalign](https://academic.oup.com/nar/article/51/W1/W17/7173809) (alignments guided by IMGT antibody numbering schemes).<br />Several options are possible to visualize and save as picture:<br /><ul><li>Click on the html file to see the alignment.<br /><ul><li>Warning: use the horizontal scrollbar of the window + the right click of the mouse on the alignment to see the left part of the representation.<br /></li><li>To change the aligments colors ("Nucleotide" recommanded for nucleotides):<br><br><br>
<div class="image-container">
  <img src="doc_images/color.png" onclick="toggleFullscreen(this)" />
</div>
<br /></li><li>To annotate the FR and CDR regions, use the .gff file in the same folder than the opened .html file:<br><br><br>
<div class="image-container">
  <img src="doc_images/gff.png" onclick="toggleFullscreen(this)" />
</div>
<br />Schematic illustration of these regions:<br><br><br>
<div class="image-container">
...
<br /></li><li>Export as picture does not work well as part of the sequence not displayed are not exported (screenshot).<br><br><br>
<ul><li>In [Jalview](https://www.jalview.org/download/).<br /><ul><li>import the .fasta file with the same name as the .html file:<br><br><br>
<div class="image-container">
  <img src="doc_images/jalview_open.png" onclick="toggleFullscreen(this)" />
</div>
<br /></li><li>Add colors:<br><br><br>
<div class="image-container">
  <img src="doc_images/jalview_color.png" onclick="toggleFullscreen(this)" />
</div>
<br /></li><li>Set a threshold to see the differences:<br><br><br>
<div class="image-container">
  <img src="doc_images/jalview_diff.png" onclick="toggleFullscreen(this)" />
</div>
<br /></li><li>A high threshold set all the columns with at least one difference in white:<br><br><br>
<div class="image-container">
  <img src="doc_images/jalview_diff_threshold_high.png" onclick="toggleFullscreen(this)" />
</div>
<br /></li><li>A low threshold set all the different minority events in each column in white:<br><br><br>
<div class="image-container">
  <img src="doc_images/jalview_diff_threshold_low.png" onclick="toggleFullscreen(this)" />
</div>
<br /></li><li>To export as .png for instance:<br><br><br>
<div class="image-container">
...
; tree scale is the number of mutations per position.
