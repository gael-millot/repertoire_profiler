...
**Clone-assigned sequences** are productive sequences with additionnal features:<br /><ul><li>germline clustering (clone ID)<br /></li><li>allele reannotation (germinal_v_call and germinal_j_call columns)<br /></li><li>mutation load<br /></li><li>distance and sequence nickname (annotation from the metadata file) added<br>
...
`r  if(params$heavy_chain == "TRUE"){"Open the different sequences_\\<SEQ_NAME\\>_align.fasta_itol_url.txt files in the [phylo](./phylo) folder and click on the link to see the trees."}`
<br><br>
...
path <- paste0(path, "/")
```
Full .nextflow.log is in: ```r path```
<br />
The one in the [reports](./reports) folder is not complete (miss the end)
