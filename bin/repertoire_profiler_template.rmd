...
<span style="color:green">*unwanted_chains.tsv file in the [tsv](./tsv) folder*</span>
```{r, echo=FALSE, results='asis'}
df <- read_tsv_with_dummy("tsv/unwanted_chains.tsv")
if (is.data.frame(df) && attr(df, "truncated")) {
  cat("\n**(Note: Table truncated to 10 rows. Full file available in the unwanted_chains.tsv file.)**\n\n")
}
kable(df, format = "markdown", row.names = FALSE)
```
</details>
Total: ```r params$nb_productive + params$nb_unproductive + params$nb_unwanted```

Productiveness ratio: ```r base::round(params$nb_productive / (params$nb_productive + params$nb_unproductive + params$nb_unwanted), 2)```

See the productive_seq.tsv, failed_productive_seq.tsv and unwanted_chains.tsv files in the [tsv](./tsv) folder, as well as their [column description](https://github.com/gael-millot/repertoire_profiler?tab=readme-ov-file#output).
...
Number of failed clone-assigned or germline sequences: ```r nb_failed_clone``` (```r nb_failed_clone_assignment``` non assigned to any clonal group and ```r nb_failed_clone_germline``` without germline sequence defined)
<br>
Total: ```r params$nb_clone_assigned + nb_failed_clone```
...
Number of sequences used in the distance matrix (from productive_seq.tsv): ```r params$nb_productive```
<br>
Number of sequences that could not be used in the distance matrix: ```r nb_dist_ignored```

Sequence ratio: ```r base::round((params$nb_productive - nb_dist_ignored)/ params$nb_productive, 2)```
