...
**Sequence Alignments** are available in the [alignments/nuc](./alignments/nuc) folder for nucleotide alignments and in the [alignments/aa](./alignments/aa) folder for amino acids alignments.

```{r, echo=FALSE, results='asis'}
if(align_soft == "abalign"){
    cat('
Such alignments are performed using [Abalign](https://academic.oup.com/nar/article/51/W1/W17/7173809) (alignments guided by IMGT antibody numbering schemes).<br>Warning: some sequences may not be aligned, because they are too short or out of the VDJ region.
<br><br>
Number of failed alignments: <code class="r-output">', nrow(failed_abalign_align) - 1, '</code>&nbsp; <details class="inline"><summary style="color: #1a73e8; font-family: Arial, sans-serif; font-size: 1em; font-weight: bold;">view</summary>
...
<br><br>
    ...
    cat('
Such alignments are performed using [Mafft](https://mafft.cbrc.jp/alignment/server/index.html).
    ')
}
```
<br><br>

Click on the desired html files to see the alignments.

<br><br>

And see [here](reports/alignments_viz.html) the ways to deal with alignments.
