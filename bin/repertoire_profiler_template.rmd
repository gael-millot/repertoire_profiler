...
read_tsv_with_dummy <- function(file_path) {
  ...
  if (file.info(file_path)$size == 0) {
    ...
    # Si le tableau est vide après lecture, ajouter une ligne vide
    if (nrow(df) == 0) {
...
render_table_with_details <- function(file_path, summary_text = "Afficher la table") {
  df <- read_tsv_with_dummy(file_path)
  cat(paste0(
    '<details class="inline"><summary style="color: #1a73e8; font-family: Arial, sans-serif; font-size: 1em; font-weight: bold;">',
    summary_text,
    '</summary>\n\n'
  ))
...
<li>
germ_tree_dismissed_seq_seq.tsv : &nbsp; <details class="inline"><summary style="color: #1a73e8; font-family: Arial, sans-serif; font-size: 1em; font-weight: bold;">view</summary>
```{r, echo=FALSE, results='asis'}
...
[comment]: <> (**Sequence alignment** displays are available in the [cdr3_multi_alignments](./cdr3_multi_alignments) folder. The HTML files present the alignment visualizations, while the FASTA files contain the corresponding sequences for each clonal group.)
...
`r  if(params$itol_subscription == "TRUE"){"Open the different sequences_\\<SEQ_NAME\\>_align.fasta_itol_url.txt files in the [phylo](./phylo) folder and click on the link to see the trees."}`
`r  if(params$itol_subscription == "FALSE"){"To see the trees, follow the link : https://itol.embl.de/upload.cgi. Choose a name for your tree (optional) and upload a '*.treefile' file from the [phylo](./phylo) folder. The trees will then be displayed by iTOL."}`

Alignment visualization is available for each clonal group used in each tree, in the [phylo_full_multi_alignments](./phylo_full_multi_alignments) folder in html format.
