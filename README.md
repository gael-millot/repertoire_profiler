...
<table style="width:100%; border-collapse:collapse; overflow-wrap: anywhere;table-layout:fixed; word-break:break-all;">
    ...
    <tr>
        ...
        <td style="white-space:normal; text-align:left; word-break:break-all; overflow-wrap:anywhere;">
            ...
            <ul style="padding-left:1.2em; margin:0;word-break:break-all; overflow-wrap:anywhere;"><li><b>for_alignment_nuc</b>: folder containing 1) a fasta file with all nucleotide sequences, and 2) one fasta file with nucleotide sequences per clonal group (i.e., sequences with the same value in the <i>clone_id</i> column of the <i>clone_assigned_seq.tsv</i> file). The kind of sequences depends on what has been selected in the <code>align_seq</code> parameter of the <i>nextflow.config</i> file.<br>Warning: fasta files can be absent for clonal groups if none have at least the number of sequences indicated in the <code>align_clone_nb</code> parameter in the <i>nextflow.config</i> file.<br>Each clonal file is named as <i>&lt;ALIGN_SEQ&gt;&#8203;_clone_id_&lt;&#8203;CLONE_ID&gt;_&lt;&#8203;V_GENE&gt;_&lt;&#8203;J_GENE&gt;&#8203;.fasta</i>.<br>The <i>germline_alignment</i> column is also added in clonal group fasta files with the following rule: 1) if all the <i>germline_alignment</i> sequences are not identical in the clonal group, the most frequent one is used and 2) if there is more than one most frequent germline sequence, then the first one is taken. See the <i>Tsv2fastaGff.log</i> file in the <i>report</i> folder. Warning: the germline sequence is always the variable region only (thus, can be shorter compared to the other sequences).<br>Sequences are made of ATGCN.
            </li><li><b>for_alignment_aa</b>: idem to <i>for_alignment_nuc</i> but using the corresponding aa columns of the <i>clone_assigned_seq.tsv</i> file. Each file is named as <i>&lt;ALIGN_SEQ&gt;&#8203;_aa_clone_id_&lt;&#8203;CLONE_ID&gt;_&lt;&#8203;V_GENE&gt;_&lt;&#8203;J_GENE&gt;&#8203;.fasta</i>. Germline sequence addition follows the same rules as in <i>for_alignment_nuc</i>, but using the corresponding aa column. Of note, even if the name of the germline sequence is the same in both the nuc and aa alignment fasta file (for instance, germline&#8203;_alignment&#8203;_clone&#8203;_id&#8203;_10&#8203;_IGHV4-34&#8203;_IGHJ6), sequences comes from 2 different columns (for instance, <i>germline_alignment</i> for nuc and <i>germline_alignment_aa</i> for aa). Sequences are made of single letter aa with X added. Warning: all the * (stop) are replaced by X to generate the <i>.fasta</i> files. Otherwise, the aligners (mafft and abalign) remove the stars, which shifts the correspondance between aa and nuc sequences.
            ...
            </li><li><b>trimmed</b>: folder of the sequences from the <i>trimmed_sequence</i> column. Sequences are made of ATGCN. Warning: empty fasta means that trimming was not possible by <code>seqkit subseq</code> because sequences in the <i>sequence_alignment</i> and <i>sequence_alignment_with_gaps</i> columns are not identical (<i>align_seq_identical</i> column is FALSE).
            ...
            </li><li><b>trimmed</b>: folder of the sequences from the <i>trimmed_sequence_aa</i> column. Sequences are made of single letter aa with X (unknown) and * (stop) added. Warning: empty fasta means that trimming was not possible by <code>seqkit subseq</code> because sequences in the <i>sequence_alignment</i> and <i>sequence_alignment_with_gaps</i> columns are not identical (<i>align_seq_identical</i> column is FALSE).
    ...
    <tr>
        ...
        <td style="white-space:normal; text-align:left; word-break:break-all; overflow-wrap:anywhere;">
            ...
            <br></li><li>sequence_aa: Translation in aa of the query nucleotide sequence performed by <code>AssignGenes.py igblast --format airr</code>. Warning: trimming is performed so that the translation starts by the aa of the v region (see the <i>aa_identical</i> column description below). Thus, beware with regions coodinates. 
            ...
            <br></li><li><b>trimmed_sequence_aa</b>: Translation in aa of the <i>sequence</i> column using <code>seqkit translate</code>. Sequences are made of single letter aa with X (unknown) and * (stop) added.
            ...
            <br></li><li>fwr1_alignment_start: FWR1 start position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>fwr1_alignment_end: FWR1 end position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>cdr1_alignment_start: CDR1 start position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>cdr1_alignment_end: CDR1 end position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>fwr2_alignment_start: FWR2 start position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>fwr2_alignment_end: FWR2 end position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>cdr2_alignment_start: CDR2 start position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>cdr2_alignment_end: CDR2 end position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>fwr3_alignment_start: FWR3 start position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>fwr3_alignment_end: FWR3 end position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>cdr3_alignment_start: CDR3 start position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>cdr3_alignment_end: CDR3 end position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>fwr4_alignment_start: FWR4 start position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
            <br></li><li>fwr4_alignment_end: FWR4 end position in both the <i>sequence_alignment</i> and <i>germline_alignment</i> fields (computed from the position difference between the <i>fwr1_start</i> and the <i>v_alignment_start</i> columns).
    ...
    <tr>
        ...
        <td style="white-space:normal; text-align:left; word-break:break-all; overflow-wrap:anywhere;">
            ...
            <br></li><li><b>clonal_germline_alignment</b>: equivalent of the <i>sequence_alignment</i> column output but for the sequence of the germline_alignment_(full | d_mask | v_region) column processed by `AssignGenes.py igblast --format airr`. This sequence should be identical to the sequence of the <i>germline_alignment_(full | d_mask | v_region)_no_gaps</i> column.
            <br></li><li><b>clonal_germline_alignment_aa</b>: equivalent of the <i>sequence_alignment_aa</i> column output but for the sequence of the germline_alignment_(full | d_mask | v_region) column processed by `AssignGenes.py igblast --format airr`. This sequence should be identical to the sequence of the <i>germline_alignment_(full | d_mask | v_region)_aa_no_gaps</i> column.
            ...
            <br></li><li><b>germline_alignment_(full | d_mask | v_region)_no_gaps</b>: as <i>germline_alignment_(full | d_mask | v_region)</i> column without IMGT gaps.
            <br></li><li><b>germline_alignment_(full | d_mask | v_region)_aa_no_gaps</b>: translation of the <i>germline_alignment_(full | d_mask | v_region)_no_gaps</i> column into amino-acids (by `Biostrings::translate()`).
            <br></li><li><b>clonal_germline_align_identical</b>: if TRUE, it means that both the sequences in the <i>germline_alignment_(full | d_mask | v_region)_no_gaps</i> and <i>clonal_germline_alignment</i> columns are identical. It FALSE, please report <a href="https://gitlab.pasteur.fr/gmillot/repertoire_profiler/-/issues">here</a>.
            <br></li><li><b>clonal_germline_align_aa_identical</b>: if TRUE, it means that both the sequences in the <i>germline_alignment_(full | d_mask | v_region)_aa_no_gaps</i> and <i>clonal_germline_alignment_aa</i> columns are identical. If FALSE, it can be because of the masked area (with <i>germline_alignment_d_mask_no_gaps</i>).
