...
| **fasta** | Folder containing all fasta files: <ul><li>**clonal_full_length_nuc** : Folder containing alignments of full length sequences from the same clonal group (e.g. with the same clone_id). Each file is named as *sequence_\<CLONE_ID\>_\<V_GENE\>-\<J_GENE\>.fasta*. The sequences are those indicated in the *sequence* and *germline_d_mask_no_gaps* columns of the *clone_assigned_seq.tsv* file. Warning: the germline_d_mask_clone_id_\<CLONE_ID\>_\<V_GENE\>-\<J_GENE\> is always the variable region only (thus, can be shorter compared to the other sequences).</li><li>**clonal_variable_nuc** : idem to *clonal_full_length_nuc* but only for the variable region of the sequences.</li><li>**clonal_full_length_aa** : Folder containing amino-acid sequences in fasta files. Each fasta file is the translation of the files in the *clonal_full_length_nuc* folder. Each file is named as *sequence_aa_\<CLONE_ID\>_\<V_GENE\>-\<J_GENE\>.fasta*.</li><li>**productive_nuc** : Folder containing fasta files of all the full length productive sequences in nucleotidic form. The sequences are those indicated in the *sequence* column of the *productive_seq.tsv* file.</li><li>**productive_aa** : Folder containing fasta files of the translated sequences in the *productive_nuc* folder. These sequences should correspond to the *sequence_aa* column of the *productive_seq.tsv* file. Warning: if stops are present (indicated by stars), please report an issue here https://gitlab.pasteur.fr/gmillot/repertoire_profiler/-/issues</li></ul> **IMPORTANT NOTE ABOUT CLONAL GROUPS** :<br>If the *clone_strategy* parameter in *nextflow.config* is set to "set", a single clonal group may contain different gene assignments. In such cases, the germline sequence included in the aligned FASTA file is selected as the most frequent germline sequence within the group.<br>If multiple germline sequences are equally frequent, the one associated with the first sequence (after sorting the group by sequence ID) is used. A warning is written to the log file whenever multiple germline sequences are detected in a group. |
