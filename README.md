...
| **fasta** | Folder containing all fasta files emitted in results : <ul><li>**aligned_aa** : Folder containing amino-acid sequences in fasta files. Each fasta file regroups the amino-acid sequences that come from the same V\|J recombination. </li><li>**aligned_nuc** : Folder containing fasta alignments of sequences from the same clonal group (e.g. with the same clone_id)</li><li>**productive_aa** : Folder containing fasta files of all producive sequences in amino-acid form.</li><li>**productive_nuc** : Folder containing fasta files of all producive sequences in nucleotidic form.</li></ul> **IMPORTANT NOTE ABOUT CLONAL GROUPS** :<br>If the *clone_strategy* parameter in *nextflow.config* is set to "set", a single clonal group may contain different gene assignments. In such cases, the germline sequence included in the aligned FASTA file is selected as the most frequent germline sequence within the group.<br>If multiple germline sequences are equally frequent, the one associated with the first sequence (after sorting the group by sequence ID) is used. A warning is written to the log file whenever multiple germline sequences are detected in a group. |
