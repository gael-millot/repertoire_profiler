...
process Gff {
    ...
    input:
    tuple path(fasta), val(tag), path(tsv) // parallelization expected (by clonal groups over align_clone_nb sequences) // warning: fasta is either nuc of aa, while other is the other one
    val align_seq
    val align_clone_nb
    path cute_file
